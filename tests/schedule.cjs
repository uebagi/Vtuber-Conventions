...
setImmediate(async () => {
  ...
  elements['#reset'].click();
  assert.equal(elements['#talent'].value, 'all'); assert.equal(count(), 263);
  ...
  assert.deepEqual(groupOptions().slice(1).map(([, name]) => name), ['a:VEnue', 'Aegis-Link', 'Algorhythm Project', 'ASTRALINE', 'BEASTIEZ', 'ChromaSHIFT', 'Digital Gear / Veizo', 'florAtelier', 'hololive', 'HUKEC', 'Oshi Connect', 'Oshi Push', 'Phase Connect', 'Variance Project', 'VirtualTakeover']);
  ...
  elements['#reset'].click();
  assert.equal(elements['#event-status'].value, 'all'); assert.equal(count(), 263);
  ...
  change('#group', 'group:ChromaSHIFT'); assert.equal(count(), 14);
  ...
  elements['#reset'].click();
  change('#group', 'group:Variance Project'); assert.equal(count(), 11);
  change('#meet-greets', 'only'); assert.equal(count(), 10);
  elements['#reset'].click();
  // New groups need only CSV data; missing fields retain the original status filters.
  run("const originalOrganizerSessions = sessions; sessions = [{...sessions[0], participants: '', event_status: 'official', organizer: ' Community & Friends ; Group, Inc. ;; Community & Friends ; '}, {...sessions[0], participants: '', event_status: 'official', organizer: 'Community & Friends'}, {...sessions[0], participants: '', event_status: 'official', organizer: 'official'}, {...sessions[0], participants: '', event_status: 'official', organizer: undefined}]; setupGroups()");
  ...
  run('sessions = [{...originalOrganizerSessions[0], participants: ""}]; delete sessions[0].organizer; setupGroups()');
  ...
  assert.equal(participantNames.size, 272);
  ...
  assert.equal(Object.values(socialData.profiles).filter(p => p.x).length, 269);
  assert.equal(Object.values(socialData.profiles).filter(p => p.primary).length, 138);
  ...
  assert.equal(count(), 263);
  change('#meet-greets', 'exclude'); assert.equal(count(), 48);
  change('#meet-greets', 'all'); assert.equal(count(), 263);
  change('#meet-greets', 'only'); assert.equal(count(), 215);
  elements['.days'].children[2].click(); assert.equal(count(), 105);
  elements['.days'].children[3].click(); assert.equal(count(), 80);
  ...
  elements['#reset'].click(); assert.equal(count(), 263);
  change('#concerts', true, true); assert.equal(count(), 19);
  change('#meet-greets', 'only'); assert.equal(elements['#concerts'].checked, false); assert.equal(count(), 215);
  change('#concerts', true, true); assert.equal(elements['#meet-greets'].value, 'exclude'); assert.equal(count(), 15);
  elements['#reset'].click(); change('#event-status', 'unofficial'); assert.equal(count(), 99);
  assert.equal(elements['#download-calendar'].disabled, false);
  change('#meet-greets', 'only'); assert.equal(count(), 96);
  ...
  elements['.days'].children[1].click(); assert.equal(count(), 25);
  elements['.days'].children[2].click(); assert.equal(count(), 39);
  elements['.days'].children[3].click(); assert.equal(count(), 32);
  ...
  elements['#reset'].click(); change('#event-status', 'official'); assert.equal(count(), 164);
  change('#meet-greets', 'only'); assert.equal(count(), 119);
  ...
  assert.equal((unfolded.match(/BEGIN:VEVENT/g) || []).length, 263);
  assert.equal(new Set(unfolded.match(/^UID:.+$/gm)).size, 263);
  assert(ics.split('\r\n').every(line => Buffer.byteLength(line, 'utf8') <= 75));
  const snapshot = JSON.parse(fs.readFileSync('conventions/vexpo-2026/sources/participants.json', 'utf8'));
  assert.equal(snapshot.length, 263); assert.equal(snapshot.filter(s => s.is_meet_greet).length, 215);
  assert.equal(snapshot.filter(s => s.event_status === 'official').length, 164);
  ...
  assert.equal(unofficial.length, 99);
  ...
  assert.equal(unofficial.filter(s => s.meet_greet_type === 'IRL').length, 11);
  ...
  assert.equal(roaming.length, 19);
  assert.equal(roaming.filter(s => s.date === '2026-09-19' && s.start_time === '14:30').length, 3);
  const roamingCalendar = run("createCalendar(sessions.filter(s => s.meet_greet_type === 'Roaming'))").replace(/\r\n /g, '');
  assert.equal(new Set(roamingCalendar.match(/^UID:.+$/gm)).size, 19);
  assert(roamingCalendar.includes('DTSTART:20260920T073000Z'));
  assert(roaming.some(s => s.participants === 'Hanakyo' && s.start_time === '11:30'));
  change('#group', 'group:Oshi Connect'); assert.equal(count(), 13);
  elements['#reset'].click();
  ...
  assert.equal(run("sessions.filter(s => s.organizer.includes('Veizo')).length"), 13);
  ...
  assert.equal(Object.keys(await run('loadSocials()')).length, 0);
  console.log('PASS: Researched direct X profiles, aliases, unknown/unsafe links, optional social data;  263 sessions; 215 meet-and-greets including 96 unofficial meet-and-greets; 19 concerts; search, status and organizer filters, reset, three-way meet-and-greet filter, calendar metadata/time conversion/unique IDs, and image removal.');
});
