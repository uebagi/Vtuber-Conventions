...
setImmediate(async () => {
  ...
  for (const [type, expected] of [['all', 226], ['concert', 16], ['meet-greet', 179], ['roaming', 9], ['afterparty', 2], ['stage-panel', 33]]) {
  ...
  for (const [query, expected, type] of [['concerts=1', 16, 'concert'], ['meet-greets=only', 179, 'meet-greet'], ['meet-greets=exclude', 47, 'exclude-meet-greets'], ['concerts=1&meet-greets=exclude', 14, 'concert'], ['type=invalid', 226, 'all']]) {
  ...
  assert.equal(run('filteredSessions().length'), 25); // Afterparty already matches Concerts.
  ...
  assert.equal(run('filteredSessions().length'), 25);
  const multiCalendar = run('createCalendar(filteredSessions())').replace(/\r\n /g, '');
  assert.equal(new Set(multiCalendar.match(/^UID:.+$/gm)).size, 25);
  ...
  check('all', true); assert.equal(run('filteredSessions().length'), 226);
  ...
  // Load the real optional announcements as a page with data-unconfirmed does.
  ...
  await run('load()');
  assert.equal(run('filteredSessions().length'), 234);
  ...
  elements['#reset'].click(); assert.equal(run('filteredSessions().length'), 234);
  ...
  elements['#reset'].click();
  assert.equal((run('createCalendar(filteredSessions())').match(/BEGIN:VEVENT/g) || []).length, 226);
  assert.equal(elements['#download-calendar'].textContent, 'Download 226 sessions (.ics)');
