...
setImmediate(async () => {
  ...
  for (const [type, expected] of [['all', 263], ['concert', 19], ['meet-greet', 215], ['roaming', 19], ['afterparty', 4], ['stage-panel', 32]]) {
  ...
  for (const [query, expected, type] of [['concerts=1', 19, 'concert'], ['meet-greets=only', 215, 'meet-greet'], ['meet-greets=exclude', 48, 'exclude-meet-greets'], ['concerts=1&meet-greets=exclude', 15, 'concert'], ['type=invalid', 263, 'all']]) {
  ...
  check('all', true); assert.equal(run('filteredSessions().length'), 263);
  ...
  await run('load()');
  assert.equal(run('filteredSessions().length'), 271);
  ...
  elements['#reset'].click(); assert.equal(run('filteredSessions().length'), 271);
  ...
  elements['#reset'].click();
  assert.equal((run('createCalendar(filteredSessions())').match(/BEGIN:VEVENT/g) || []).length, 263);
  assert.equal(elements['#download-calendar'].textContent, 'Download 263 sessions (.ics)');
