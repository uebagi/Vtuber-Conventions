...
setImmediate(async () => {
  ...
  for (const [type, expected] of [['all', 261], ['concert', 19], ['meet-greet', 213], ['roaming', 19], ['afterparty', 4], ['stage-panel', 32]]) {
  ...
  for (const [query, expected, type] of [['concerts=1', 19, 'concert'], ['meet-greets=only', 213, 'meet-greet'], ['meet-greets=exclude', 48, 'exclude-meet-greets'], ['concerts=1&meet-greets=exclude', 15, 'concert'], ['type=invalid', 261, 'all']]) {
  ...
  assert.equal(run('filteredSessions().length'), 39); // Concerts and afterparties overlap; lounge music only matches Afterparty.
  ...
  assert.equal(run('filteredSessions().length'), 39);
  const multiCalendar = run('createCalendar(filteredSessions())').replace(/\r\n /g, '');
  assert.equal(new Set(multiCalendar.match(/^UID:.+$/gm)).size, 39);
  ...
  check('all', true); assert.equal(run('filteredSessions().length'), 261);
  ...
  // The published announcements are now timed; retain a historical fixture to exercise pending UI.
  context.document.body.dataset.unconfirmed = 'unconfirmed.json';
  assert.equal((await run('loadUnconfirmed()')).length, 0);
  const liveFetch = context.fetch;
  context.fetch = async path => path === 'unconfirmed.json'
    ? {ok: true, json: async () => JSON.parse(fs.readFileSync('tests/fixtures/unconfirmed.json', 'utf8'))}
    : liveFetch(path);
  const pending = await run('loadUnconfirmed()');
  assert.equal(pending.length, 8);
  const nephla = pending.find(s => s.participants === 'NEPHLA');
  assert.equal(nephla.is_concert, 'true'); assert.equal(nephla.is_meet_greet, 'false');
  assert.equal(nephla.start_time, '');
  const nana = pending.find(s => s.participants === 'Nana Asteria');
  ...
  await run('load()');
  assert.equal(run('filteredSessions().length'), 269);
  ...
  assert.equal(new URL(context.location.href).searchParams.get('day'), 'unconfirmed');
  assert.equal(elements['#download-calendar'].disabled, true);
  ...
  elements['#reset'].click(); assert.equal(run('filteredSessions().length'), 269);
  assert.equal(new URL(context.location.href).search, '');
  context.location.href = sharedPending; windowHandlers.popstate();
  assert.equal(run('filteredSessions().length'), 8);
  select('meet-greet'); assert.equal(run('filteredSessions().length'), 7);
  elements['#group'].value = 'group:Digital Gear / Veizo'; elements['#group'].handlers.change();
  assert.equal(run('filteredSessions().length'), 7);
  elements['#talent'].value = run("talentKey('Mikan')"); elements['#talent'].handlers.change();
  assert.equal(run('filteredSessions().length'), 1);
  assert.equal(run("participantChip('Mikan').href"), 'https://x.com/MikanVT');
  elements['#reset'].click();
  elements['.days'].children.find(b => b.dataset.day === '2026-09-19').click();
  assert.equal(run('filteredSessions().filter(s => !hasConfirmedTime(s)).length'), 0);
  assert.equal(run("filteredSessions().filter(s => s.organizer === 'Digital Gear / Veizo').length"), 6);
  elements['#reset'].click();
  assert.equal((run('createCalendar(filteredSessions())').match(/BEGIN:VEVENT/g) || []).length, 261);
  assert.equal(elements['#download-calendar'].textContent, 'Download 261 sessions (.ics)');
  ...
  await assert.rejects(run('loadUnconfirmed()'), /unavailable/);
  context.fetch = async () => ({ok: true, json: async () => ({sessions: [{event: 'Bad data'}]})});
  await assert.rejects(run('loadUnconfirmed()'), /Invalid/);
  delete context.document.body.dataset.unconfirmed;
  assert.equal((await run('loadUnconfirmed()')).length, 0);
